...
def caption_generator(image):
    caption_model = load_model('static/models/image_caption_model_cow.h5')
    ...
    def create_feature_extractor():
        ...
        x = base_model.output
        x = tf.keras.layers.Dense(4096, activation='relu')(x)  # Adjust to output the required shape
    ...
    def extract_vgg16_features(image):
        ...
        extracted_features = feature_extractor.predict(image)
        return extracted_features

    def predict_caption_h5(image_features, tokenizer, max_length):
        in_text = 'startsentence'
        for i in range(max_length):
            sequence = tokenizer.texts_to_sequences([in_text])[0]
            sequence = pad_sequences([sequence], maxlen=max_length)
            ...
            yhat = caption_model.predict([np.array(image_features).reshape((1, 4096)), sequence], verbose=0)
            yhat = np.argmax(yhat)
    ...
    image_features = extract_vgg16_features(image)
    caption = predict_caption_h5(image_features, tokenizer, max_length)
